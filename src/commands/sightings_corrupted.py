...
def setup_sightings_commands(bot):
    ...
    async def sightingsseen(interaction: discord.Interaction):
        ...
        if user_count == 0:
            user_stats = "**No sightings yet**\nStart reacting to UFO images with 👽"
        else:
            user_stats = f"**{user_count:,}** confirmed sightings\nServer rank: **{rank_text}**"
        ...
        if user_count == 0:
            footer_msg = "Ready to start your alien hunting journey? 🛸"
        elif user_count < 5:
            ...
            footer_msg = "You are a true believer! The truth is out there! 🌟"
    ...
    async def globalsightings(interaction: discord.Interaction):
        ...
        if user_server_breakdown:
            ...
            embed.add_field(
                name="Sightings by Server",
                value="\n".join(breakdown_lines[:5]) + ("..." if len(breakdown_lines) > 5 else ""),
                inline=False
            )
        ...
        if total_global_sightings > 0:
            ...
            global_stats = (
                ...
                f"�️ **{total_servers}** servers with activity\n"
